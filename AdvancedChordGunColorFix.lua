-- @description Advanced ChordGun (Colorblind-Friendly)
-- @author del-paso
-- @version 2.0
-- @about
--   ### Advanced ChordGun v2 (Colorblind-Friendly Edition)
--   Uses the Wong palette (blue/orange) for suggestion colors and status
--   indicators. All color distinctions rely on hue AND lightness differences,
--   safe for protanopia and deuteranopia (red-green color deficiency).
...
local SECTION = "com.del-paso.AdvancedChordGun.CB"
local PROJ = 0
...
local function getSuggestionColor(score)
  if score >= 0.55 then return {0.00,0.35,1.00,1.0} end   -- vivid royal blue (best)
  if score >= 0.42 then return {0.00,0.75,1.00,1.0} end   -- cyan-blue (good)
  if score >= 0.30 then return {1.00,0.65,0.00,1.0} end   -- amber orange (decent)
  if score >= 0.20 then return {0.75,0.35,0.00,1.0} end   -- burnt orange (possible)
  return nil
end
...
local C = {
  ...
  gridCell={0.18,0.18,0.25}, gridHover={0.26,0.26,0.36}, gridDia={0.15,0.32,0.50},
  headerRow={0.14,0.14,0.20}, catText={0.60,0.50,0.80}, sep={0.28,0.28,0.35},
  green={0.00,0.45,0.70}, red={0.84,0.37,0.00}, orange={0.90,0.62,0.00},
        ...
        for deg=1,7 do
          ...
          if cellVis then
            ...
            if suggestionCache[ri] and suggestionCache[ri][deg] then
              ...
              if sugCol then gfx.set(sugCol[1],sugCol[2],sugCol[3],1.0); gfx.rect(cx,drawY,colW,rowH,true) end
            end
...
local sugScore = suggestionCache[ri] and suggestionCache[ri][deg] or 0

-- Dark text on bright cells
if sugScore >= 0.20 then
    sc({0.12,0.12,0.12}) -- near black
else
    sc(C.text) -- normal white/light text
end
...
local function drawBottomPanel(panelY)
  ...
  sc(clrHov and {0.84,0.37,0.00} or C.btnNorm); drawRR(clrBtnX,resultY,clrBtnW,clrBtnH,3,true)
...
local function init()
  ...
  gfx.init("Advanced ChordGun (CB-Friendly)", w, h, 0, x, y)
